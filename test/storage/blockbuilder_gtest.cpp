...
TEST(BlockBuilderTest, empty_test) {
    ...
    EXPECT_EQ(sizeof(uint32_t) * 3, block_builder.estimated_size());
}


TEST(BlockBuilderTest, simple_test) { 
    const int N = 1024;
    uint32_t expect_size = sizeof(uint32_t) * 2;
    for(int i = 0; i < N; i++) {
        if(i % Option::RESTART_INTERVAL == 0) {
            expect_size += sizeof(uint32_t);
        }
        std::string key(i + 1, 'a');
        std::string value(i + 1, 'b');
        expect_size += key.size() + value.size() + sizeof(uint32_t) * 2;
