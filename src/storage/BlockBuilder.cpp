...
void BlockBuilder::add(const std::string &key, const std::string &value) {
    ...
    buffer_.append((char*)&key_size, sizeof(uint32_t));
    buffer_.append(key);
    buffer_.append((char*)&value_size, sizeof(uint32_t));
...
std::string BlockBuilder::finish() {
    uint32_t restarts_postion = buffer_.size();
    uint32_t num = restarts_.size();
    // append size
    buffer_.append((char*)&num, sizeof(uint32_t));
    // append restarts_
    for(auto i : restarts_){
        buffer_.append((char *)&i, sizeof(uint32_t));
    }
    // append postion
    buffer_.append((char *)&restarts_postion, sizeof(uint32_t));
    // finished
    finished_ = true;
